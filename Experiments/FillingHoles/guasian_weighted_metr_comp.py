...
def warpImages(img1, img2, H):
    ...
    def fill_black_pixels_gaussian(image):
        """
        Заповнює чорні пікселі методом Gaussian Weighted Blending.

        Args:
            image: Вхідне зображення із чорними пікселями.

        Returns:
            Зображення з виправленими чорними пікселями.
        """
        ...
        # Створюємо Gaussian Kernel
        kernel_size = 5
        sigma = 2.0
        gauss_kernel = cv2.getGaussianKernel(kernel_size, sigma)
        gauss_kernel = gauss_kernel @ gauss_kernel.T  # 2D ядро

        # Для кожного каналу (BGR)
        for channel in range(3):
            channel_data = image[:, :, channel].astype(np.float32)

            # Замінюємо чорні пікселі на 0
            channel_data[inner_black_pixels_mask == 255] = 0

            # Застосовуємо Гауссовий фільтр для сусідів
            weighted_values = gaussian_filter(channel_data, sigma=sigma)
            weights = gaussian_filter((inner_black_pixels_mask == 0).astype(np.float32), sigma=sigma)

            # Нормалізуємо і оновлюємо чорні пікселі
            filled_values = weighted_values / (weights + 1e-5)  # Захист від ділення на 0
            channel_data[inner_black_pixels_mask == 255] = filled_values[inner_black_pixels_mask == 255]
            image[:, :, channel] = np.clip(channel_data, 0, 255).astype(np.uint8)

        # Зберігаємо зображення з виправленими чорними пікселями
        cv2.imwrite("image_with_filled_black_pixels_gaussian.jpg", image)

        return image


    output_img = fill_black_pixels_gaussian(output_img)
