...
def warpImages(img1, img2, H):
    ...
    def find_inner_black_pixels(image, base_image):
        """
        Заповнює чорні пікселі методом inpainting OpenCV (Telea/Navier-Stokes).
        
        Args:
            image: Зображення після трансформації, з артефактами.
            base_image: Базове зображення (до трансформації).
        ...
        # Визначаємо координати чорних пікселів
        holes_coords = np.argwhere(inner_black_pixels_mask == 255)

        # Створюємо маску для OpenCV inpainting
        inpaint_mask = (inner_black_pixels_mask > 0).astype(np.uint8)

        # Виконуємо inpainting
        filled_image = cv2.inpaint(image, inpaint_mask, inpaintRadius=1, flags=cv2.INPAINT_TELEA)

        # Зберігаємо зображення
        cv2.imwrite("image_with_filled_black_pixels.jpg", filled_image)
